...
def order_to_text(order: Dict) -> str:
    ...
    order_text += f"\nСоздан: {order.get("created_at") if order.get("created_at") is not None else '_'}\n"
    order_text += f"Срок до: {order.get("deadline") if order.get("deadline") is not None else '_'}\n"
